"""DEMO ONLY — district terrain builder for the standalone dashboard.

Defect 8: this script requires india_districts.geojson, which is not in the
repository, has no download step and no recorded provenance. geo_data.json is
therefore SHIPPED BUT NOT REPRODUCIBLE, and we cannot state the licence of its
boundaries — which M1 requires per district.

It also folds ne_india_locations.json in as "ground-truth overrides". Those
values were generated with random.uniform (defect 5), so the randomness has
propagated into geo_data.json and from there into the demo dashboard.

Do not use this script's output as data. M1 replaces it with real NER district
boundaries from a named, licensed source.
"""
import os as _os

_HERE = _os.path.dirname(_os.path.abspath(__file__))
if not _os.path.exists(_os.path.join(_HERE, "india_districts.geojson")):
    raise SystemExit(
        "\nMISSING INPUT: india_districts.geojson\n\n"
        "This file is not in the repository and its licence is unrecorded, so\n"
        "geo_data.json cannot currently be reproduced. Before regenerating it,\n"
        "the boundary source must be decided and recorded (see ARCHITECTURE.md\n"
        "section 12, open item 1). Candidates: GADM v4.1 level-2 (free,\n"
        "non-commercial, attribution required) or Survey of India / data.gov.in.\n"
    )
...
REPO = os.path.join(BASE_DIR, "..", "..", "backend", "data", "ne_india_locations.json")
