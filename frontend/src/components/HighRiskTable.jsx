...
export default function HighRiskTable({ locations = [], onSelectLocation, onViewAll }) {
  // Filter out locations where hazard_index is null so unmonitored districts are not falsely labeled
  const monitoredLocations = (locations || []).filter(
    (loc) => loc && (loc.hazard_index != null || loc.risk_probability != null)
  );

  // Sort high risk locations first using hazard_index or risk_probability
  const highRiskSorted = [...monitoredLocations]
            ...
            <tbody className="divide-y divide-slate-100">
              {highRiskSorted.length === 0 ? (
                ...
                highRiskSorted.map((loc) => {
                  ...
                  const isLow = loc.risk_category === 'Low' || (prob != null && prob < 30);
                  const displayCategory = loc.risk_category || (prob != null ? (isHigh ? 'High' : isLow ? 'Low' : 'Moderate') : '--');
                        ...
                        <span className="inline-flex items-center gap-1 text-[11px] font-semibold">
                          <span
                            className={`w-2 h-2 rounded-full ${
                              isHigh ? 'bg-red-500' : isLow ? 'bg-emerald-500' : 'bg-amber-500'
                            }`}
                          />
                          <span className={isHigh ? 'text-red-600' : isLow ? 'text-emerald-600' : 'text-amber-600'}>
                            {displayCategory}
                          </span>
                        </span>
                      </td>
